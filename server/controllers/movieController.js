...
export const uploadTrailer = async (req, res) => {
  try {
    ...
    if (!file) return res.status(400).json("video file is missing!");
    ...
    // console.log(error)
    res.status(400).json({message: "error while uploading trailer", error})
  }
};
...
export const createMovie = async (req, res) => {
  ...
  if (director) {
    if (!isValidObjectId(director))
      return res.status(400).json("Invalid director id!");
    newMovie.director = director;
  }

  if (writers) {
    for (let w of writers) {
      if (!isValidObjectId(w))
        return res.status(400).json("Invalid writer Id!");
    }
